...
int dfs(TreeNode* root) {
	...
	int longestLenViaRoot = findHeight(root->left) + findHeight(root->right) + 2;

	return max({leftDia, rightDia, longestLenViaRoot});

	// return max(leftDia, max(rightDia, longestLenViaRoot));
