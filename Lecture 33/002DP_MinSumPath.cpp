...
// time : O(mn)
// space: mn due to dp[][] + (m+n) due to call stack
...
int fBottomUp(const vector<vector<int>>& grid,
              int m, int n) {
	...
	// constructing path from dp[][]

	int x = 0; int y = 0;

	while (!(x == m - 1 and y == n - 1)) {
		cout << x << " " << y << endl;
		if (y + 1 < n and dp[x][y] == dp[x][y + 1] + grid[x][y]) {
			y++;
		} else {
			x++;
		}
	...
	return dp[0][0]; // at the 0,0th index we store f(0, 0)
}

// time : O(mn)
// space: O(n) due to dp[]

int fBottomUpSpaceOptimised(const vector<vector<int>>& grid,
                            int m, int n) {
	...
	// at this point of time dp[] holds the 0th row of dp[][]
