...
int main() {
	...
	for (int x : v) { // range-based for loop // for-each loop
		// here, in each iteration an element of v is copied into x
		cout << x << " ";
	}
	...
	for (int& x : v) { // range-based for loop // for-each loop
		// here, x is a reference to the current element of v
		cout << x << " ";
	}

	cout << endl;

	for (auto x : v) { // range-based for loop // for-each loop
		cout << x << " ";
	}

	cout << endl;

	v.pop_back(); // 50 is popped

	cout << "size : " << v.size() << endl; // 4
	cout << "capacity : " << v.capacity() << endl; // 8

	for (auto x : v) { // 10 20 30 40
		cout << x << " ";
	}
